...
if __name__ == "__main__":
    ...
    robot = Robot()
    #time.sleep(5)
    init_pose = [160.27, 108.67, 429.75, 0.3633, 1.4682, 0.4227]
    ...
    drill_fun_global_liang = tuple([490.32 + 150, 267.83 + 40, 421.57 + 40])
    drill_fun_global_liang = np.array(drill_fun_global_liang)
    # 量出来的物体上小拇指对应关键点
    drill_xiao_global_liang = tuple([490.32 + 150, 267.83 + 40, 421.57 - 40])
    drill_xiao_global_liang = np.array(drill_xiao_global_liang)
    # 量出来的物体上手腕对应真实关键点
    drill_shouwan_true_global_liang = tuple([490.32, 267.83, 421.57])
    drill_shouwan_true_global_liang = np.array(drill_shouwan_true_global_liang)
    # 计算手与工具间的变换矩阵,先求解手与末端之间的变换矩阵
    # 建立物体坐标系，用于初始化计算
    # 两个基轴
    origin_o_x_liang = drill_fun_global_liang - drill_shouwan_true_global_liang
    origin_o_y_liang = drill_xiao_global_liang - drill_shouwan_true_global_liang
    # 标准化
    ...
    # a = 1
    # #物体上功能手指对应位置
    drillfunc_center = kp[0]
    drillf_x, drillf_y = drillfunc_center[0].item(), drillfunc_center[1].item()
    # drillf_x = int(round(drillf_x * (640 / 448)))
    # drillf_y = int(round(drillf_y * (480 / 448)))
    drillf_y1 = drillf_y
    drillf_x1 = drillf_x
    drillf_x = int(round(drillf_y1 * (640 / 448)))
    drillf_y = int(round(drillf_x1 * (480 / 448)))
    drillfunc_center = [drillf_x, drillf_y]
    # 将目标像素xy坐标转换至相机坐标系下
    # 获取对齐的图像与相机内参
    drilldis_fun = aligned_depth_frame.get_distance(drillf_x, drillf_y)
    drillcamera_xyz_fun = rs.rs2_deproject_pixel_to_point(
        depth_intrin, drillfunc_center, drilldis_fun)  # 计算相机坐标系的xyz

    drillmatrix3_fun = np.array(
        [[drillcamera_xyz_fun[0]], [drillcamera_xyz_fun[1]], [drillcamera_xyz_fun[2]], [1]])
    drill_fun_global = np.array(calculate_result_matrix(drillmatrix3_fun))
    drill_fun_global[0] = drill_fun_global[0] * 1000
    drill_fun_global[1] = drill_fun_global[1] * 1000
    drill_fun_global[2] = drill_fun_global[2] * 1000

    # 物体上小拇指对应位置
    drillxiao_center = kp[1]
    drillxiao_x, drillxiao_y = drillxiao_center[0].item(), drillxiao_center[1].item()
    drillxiao_x1 = drillxiao_x
    drillxiao_y1 = drillxiao_y
    drillxiao_x = int(drillxiao_y1 / 448 * 640)
    drillxiao_y = int(drillxiao_x1 / 448 * 480)
    drillxiao_center = [drillxiao_x, drillxiao_y]
    # 将目标像素xy坐标转换至相机坐标系下
    # 获取对齐的图像与相机内参
    drilldis_xiao = aligned_depth_frame.get_distance(drillxiao_x, drillxiao_y)
    drillcamera_xyz_xiao = rs.rs2_deproject_pixel_to_point(
        depth_intrin, drillxiao_center, drilldis_xiao)  # 计算相机坐标系的xyz

    drillmatrix3_xiao = np.array(
        [[drillcamera_xyz_xiao[0]], [drillcamera_xyz_xiao[1]], [drillcamera_xyz_xiao[2]], [1]])
    drill_xiao_global = np.array(calculate_result_matrix(drillmatrix3_xiao))
    drill_xiao_global[0] = drill_xiao_global[0] * 1000
    drill_xiao_global[1] = drill_xiao_global[1] * 1000
    drill_xiao_global[2] = drill_xiao_global[2] * 1000

    # 物体上手腕对应投影点
    drillshouwan_center = kp[2]
    drillshouwan_x, drillshouwan_y = drillshouwan_center[0].item(), drillshouwan_center[1].item()
    drillshouwan_x1 = drillshouwan_x
    drillshouwan_y1 = drillshouwan_y
    drillshouwan_x = int(drillshouwan_y1 / 448 * 640)
    drillshouwan_y = int(drillshouwan_x1 / 448 * 480)
    drillshouwan_center = [drillshouwan_x, drillshouwan_y]
    # 将目标像素xy坐标转换至相机坐标系下
    # 获取对齐的图像与相机内参
    drilldis_shouwan = aligned_depth_frame.get_distance(drillshouwan_x, drillshouwan_y)
    drillcamera_xyz_shouwan = rs.rs2_deproject_pixel_to_point(
        depth_intrin, drillshouwan_center, drilldis_shouwan)  # 计算相机坐标系的xyz

    drillmatrix3_shouwan = np.array(
        [[drillcamera_xyz_shouwan[0]], [drillcamera_xyz_shouwan[1]], [drillcamera_xyz_shouwan[2]], [1]])
    drill_shouwan_global = np.array(calculate_result_matrix(drillmatrix3_shouwan))
    drill_shouwan_global[0] = drill_shouwan_global[0] * 1000
    drill_shouwan_global[1] = drill_shouwan_global[1] * 1000
    drill_shouwan_global[2] = drill_shouwan_global[2] * 1000
    #
    #
    ...
    # p=tuple([110,40,0])
    # p=np.array(p)
    # drill_shouwan_true_global = drill_shouwan_global + p
    ...
    len_L_W = 160
    # 计算生成的点0与点1的向量0->1
    xiangliang_0_1 = drill_xiao_global - drill_fun_global
    # 归一化
    xiangliang_0_1 = xiangliang_0_1 / np.linalg.norm(xiangliang_0_1)
    # 求更新后的点1，作为新的小拇指关键点
    drill_xiao_global = drill_fun_global + xiangliang_0_1 * len_F_L
    # 取出更新后的小拇指关键点，分别为x_1_pie  y_1_pie  z_1_pie
    x_1_pie = drill_xiao_global[0]
    y_1_pie = drill_xiao_global[1]
    z_1_pie = drill_xiao_global[2]
    # 向量 0->1‘
    xiangliang_0_1_pie = drill_xiao_global - drill_fun_global
    ...
    # 向量 0->2
    xiangliang_0_2 = drill_shouwan_global - drill_fun_global
    # 计算0 1’ 2构成的平面的法向量
    ...
    # 取出功能关键点x_0 y_0 z_0
    x_0 = drill_fun_global[0]
    y_0 = drill_fun_global[1]
    z_0 = drill_fun_global[2]
    ...
    # 求解出手腕真实的关键点对应的世界坐标
    drill_shouwan_true_global = tuple([1, 1, 1])
    drill_shouwan_true_global = np.array(drill_shouwan_true_global)
    drill_shouwan_true_global[0] = a_0_1_pie / b_0_1_pie * (((A - G) / (2 * E)) + B / b_0_1_pie) / (F / (2 * E) + a_0_1_pie / b_0_1_pie) - B / b_0_1_pie
    drill_shouwan_true_global[1] = (((A - G) / (2 * E)) + B / b_0_1_pie) / (F / (2 * E) + a_0_1_pie / b_0_1_pie)
    drill_shouwan_true_global[2] = (m * n_1 + b_0_1_pie * z_0 - c_0_1_pie * y_0) / b_0_1_pie + c_0_1_pie / b_0_1_pie * (((A - G) / (2 * E)) + B / b_0_1_pie) / (F / (2 * E) + a_0_1_pie / b_0_1_pie)

    #微调一下
    drill_shouwan_true_global[2] = drill_shouwan_true_global[2] - 100
    a = 1

    #小验证，验证E H的正确性
    # drill_fun_global = tuple([491.48 + 150, 255.31 + 50, 417.16 + 40])
    # drill_fun_global = np.array(drill_fun_global)
    # drill_xiao_global = tuple([491.48 + 150, 255.31 + 40, 417.16 - 40])
    # drill_xiao_global = np.array(drill_xiao_global)
    # drill_shouwan_true_global = tuple([491.48, 264.8, 417.16+20])
    # drill_shouwan_true_global = np.array(drill_shouwan_true_global)
    ...
    # #------------------------------------建立物体坐标系--------------------------
    # 两个基轴
    origin_o_x = drill_fun_global - drill_shouwan_true_global
    origin_o_y = drill_xiao_global - drill_shouwan_true_global
    # 标准化
    ...
    #time.sleep(7)
    #机械手平移过去
    tool_pingyi = [drill_shouwan_true_global[0],drill_shouwan_true_global[1],drill_shouwan_true_global[2],euler_liang[0],
                 euler_liang[1], euler_liang[2]]
