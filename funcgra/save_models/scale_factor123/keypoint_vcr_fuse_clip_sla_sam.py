...
class Net(nn.Module):
    def __init__(self, aff_classes=6):
        ...
        self.keypoint_weight = nn.Parameter(torch.randn(6, 9))  # 可学习的权重
    ...
    def func_test_forward(self, ego, aff_label, point):
        ...
        for i in range(bs):
            ...
            kp.append(final_keypoints)

        all_affgrounding = torch.stack(sample_ego_kp_features)
        all_kp = torch.stack(kp)
        return all_affgrounding, all_kp
    ...
    def get_ego_mask(self, ego, point):
        ...
        # self.visualize_each_mask(ret["rgb"], ret["seg"])
        return ret

    def visualize_each_mask(self, rgb, labeled_masks, save_dir="output_masks/"):
        """
        Generate separate visualization images for each mask label.

        Args:
            rgb (np.ndarray): Original RGB image, shape (H, W, 3).
            labeled_masks (np.ndarray): Segmentation mask, shape (H, W), with labels 1, 2, 3, etc.
            save_dir (str): Directory to save individual mask visualizations.
        """
        import os
        os.makedirs(save_dir, exist_ok=True)  # 创建保存目录

        # 获取掩码中的唯一类别（排除背景 0）
        unique_labels = np.unique(labeled_masks)
        unique_labels = unique_labels[unique_labels > 0]  # 过滤掉背景

        # 定义颜色映射（不同类别对应不同颜色）
        colormap = plt.cm.get_cmap("jet", len(unique_labels) + 1)

        # 遍历每个类别，单独生成一张图片
        for i, label in enumerate(unique_labels):
            mask = labeled_masks == label  # 当前类别的掩码

            # 生成空白背景（黑色）
            color_mask = np.zeros_like(rgb, dtype=np.uint8)

            # 获取颜色（归一化到 [0,1] 之间）
            color = colormap(i / len(unique_labels))[:3]  # 取RGB前三维

            # 将颜色值缩放到 [0,255] 并应用掩码
            color_mask[mask] = (np.array(color) * 255).astype(np.uint8)

            # 叠加在原图上，增强可视化效果
            alpha = 0.6  # 透明度参数
            overlay = cv2.addWeighted(rgb, 1 - alpha, color_mask, alpha, 0)

            # 保存每个类别的分割结果
            save_path = os.path.join(save_dir, f"mask_label_{label}.png")
            plt.figure(figsize=(6, 6))
            plt.imshow(overlay)
            plt.axis("off")
            plt.title(f"Segmentation Mask - Label {label}")
