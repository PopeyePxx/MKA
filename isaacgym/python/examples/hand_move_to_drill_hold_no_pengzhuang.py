...
sim_params.gravity = gymapi.Vec3(0.0, 0.0, -11.8)
...
drill_asset_options.fix_base_link = True  # 根据需要设置是否固定基座
...
hand_pose.p = gymapi.Vec3(1.2, 1.5, 0.25)  # 设置手的位置
...
quart_x_drill=gymapi.Quat.from_axis_angle(gymapi.Vec3(1,0,0),-math.pi/2) #设置电钻的姿态
quart_y_drill=gymapi.Quat.from_axis_angle(gymapi.Vec3(0,1,0),math.pi/2) #设置电钻的姿态

final = quart_x * quart_z
hand_pose.r = final
drill_final=quart_y_drill*quart_x_drill
...
drill_pose.p = gymapi.Vec3(1.2, 1, 0.5)  # 设置电钻的位置
drill_pose.r = drill_final #设置电钻的姿态
...
#
# # 启用自碰撞检测
# for i in range(len(drill_shape_props)):
#     drill_shape_props[i].filter = 0b0010 | (i << 2)  # 每个刚体都有不同的过滤器值
#
# gym.set_actor_rigid_shape_properties(env, hand_handle, hand_shape_props)
# gym.set_actor_rigid_shape_properties(env, drill_handle, drill_shape_props)
# 获取手部关节属性并配置
#hand_dof_props = gym.get_actor_dof_properties(env, hand_handle)

# # 打印关节信息（调试用）
# print("\n=== 手部关节信息 ===")
# for i, name in enumerate(gym.get_actor_dof_names(env, hand_handle)):
#     print(f"关节 {i}: {name}")
#     print(f"当前属性: 驱动模式={hand_dof_props['driveMode'][i]}")
#     print(f"          刚度={hand_dof_props['stiffness'][i]}")
#     print(f"          阻尼={hand_dof_props['damping'][i]}")
#
# # 配置所有关节为位置控制模式
# hand_dof_props['driveMode'][:] = gymapi.DOF_MODE_POS
# hand_dof_props['stiffness'][:] = 800.0  # 增大刚度值
# hand_dof_props['damping'][:] = 100.0     # 适当阻尼
# hand_dof_props['velocity'][:] = 2.0      # 最大速度限制
# hand_dof_props['effort'][:] = 500.0      # 力矩限制
#
# # 应用修改后的属性
...
# 主控制参数
#target_positions = np.zeros(len(hand_dof_props['names']))
#current_positions = np.zeros_like(target_positions)
# close_speed = 0.5  # 弧度/秒
# is_closing = False
...
# # 在查看器中添加控制提示
# gym.subscribe_viewer_keyboard_event(viewer, gymapi.KEY_SPACE, "toggle_grip")
...
#物体上功能手指关键点
drill_position_F_O=list(drill_rb_states['pose']['p'][4])
drill_position_F_O[0]-=0.025
drill_position_F_O[1]+=0.01
drill_position_F_O[2]-=0.015
drill_position_F_O=tuple(drill_position_F_O)

#物体上小拇指关键点
drill_position_L_O=list(drill_rb_states['pose']['p'][2])
drill_position_L_O[0]-=0.02
drill_position_L_O[1]+=0.01
drill_position_L_O[2]+=0.01

drill_position_L_O=tuple(drill_position_L_O)

#物体上手腕关键点
drill_position_W_O_T=list(drill_rb_states['pose']['p'][2])
drill_position_W_O_T[0]+=0.01
drill_position_W_O_T[1]+=0.005
drill_position_W_O_T[2]+=0.0
drill_position_W_O_T=tuple(drill_position_W_O_T)

#物体上手腕关键点对应的真实关键点
drill_position_W_O=list(drill_rb_states['pose']['p'][2])
drill_position_W_O[0]+=0.01
drill_position_W_O[1]+=0.07
drill_position_W_O[2]-=0.015
...
add_marker(env,drill_position_W_O_T)
...
def move_hand_to_target(hand_pose, rotation_O_H, pingyi_O_H_O):
    ...
    hand_pose.p = gymapi.Vec3(
        hand_pose.p.x - pingyi_O_H_I[0]+0,
        hand_pose.p.y - pingyi_O_H_I[1]+0.02,
...
lower_limits[shizhi_guanjie1]-=0.06
lower_limits[shizhi_guanjie2]-=0.5
...
while not gym.query_viewer_has_closed(viewer):
    ...
    draw_coordinate_frame(gym, viewer, env, drill_position, drill_rotation_matrix, color=(1, 0, 0,0,1,0,0,0,1))  # 红色

    # 获取手部的所有关节信息
    hand_joints = gym.get_actor_dof_properties(env, hand_handle)

    # 打印所有关节的名字和索引，帮助你找到对应手指的关节
    # for i in range(len(hand_joints['name'])):
    #     print(f"Joint {i}: {hand_joints['name'][i]}")

    # 假设我们已经知道了对应于特定手指的关节索引
    # 例如，index_finger_joint_index 是食指的第一个关节索引
    # index_finger_joint_index = 4  # 这个值需要根据实际情况调整
    #
    # # 设置目标角度，单位为弧度
    # target_angle = 5  # 根据实际需求调整这个值
    #
    # # 使用gym.set_dof_target_position设置目标角度
    # gym.set_dof_target_position(env, index_finger_joint_index, target_angle)

    # 如果需要让手指逐渐闭合，可以循环调用simulate和fetch_results
    # for _ in range(5):  # 模拟100步
    gym.simulate(sim)
    gym.fetch_results(sim,True)
...
#         if event.action == "toggle_grip" and event.value > 0:
#             is_closing = not is_closing
#             print(f"\n手部状态切换：{'闭合' if is_closing else '张开'}")
#
#     # 更新目标位置
#     if is_closing:
#         # 示例：同时控制多个手指
#         target_positions[4] = 0.8  # 食指
#         target_positions[6] = 0.8  # 中指
#         target_positions[8] = 0.8  # 无名指
#         target_positions[10] = 0.8  # 小指
#         target_positions[2] = 0.8  # 拇指
#     else:
#         target_positions[:] = 0.0
#
#     # 渐进式更新实现平滑运动
#     current_positions = np.minimum(
#         current_positions + close_speed * gym.get_sim_time_step(sim),
#         target_positions
#     )
#
#     # 设置关节目标位置
...
#         env,
#         hand_handle,
#         current_positions.tolist()import isaacgym
# from isaacgym import gymapi, gymutil,gymtorch
# import math
# import numpy as np
# from scipy.spatial.transform import Rotation as R
...
# # 加载手的资产
# hand_asset_root = "/home/hun/IsaacGym_Preview_4_Package/isaacgym/yinshi_URDF_RL/urdf-five_R/robots"
# hand_asset_file = "urdf-five3.xml"
# hand_asset_options = gymapi.AssetOptions()
# hand_asset_options.fix_base_link = True
# hand_asset_options.armature = 0.01
# hand_asset = gym.load_asset(sim, hand_asset_root, hand_asset_file, hand_asset_options)
...
# drill_position_W_O=list(drill_rb_states['pose']['p'][2])
# drill_position_W_O[0]+=0.01
# drill_position_W_O[1]+=0.03
# drill_position_W_O[2]+=0.0
#
# drill_position_W_O=tuple(drill_position_W_O)
#
#
...
#     # 应用平移
#     hand_pose.p = gymapi.Vec3(
#         hand_pose.p.x - pingyi_O_H_I[0]+0.01,
#         hand_pose.p.y - pingyi_O_H_I[1]+0.05,
...
# #
# #     # 可视化当前关节角度
# #     current_angles = dof_states[:, 0].cpu().numpy()
# #     if gym.get_frame_count(sim) % 60 == 0:
# #         print("\n当前关节角度：")
# #         for i, name in enumerate(gym.get_actor_dof_names(env, hand_handle)):
# #             print(f"{name}: {current_angles[i]:.2f} rad")
# #
# #     # 绘制坐标系（保持原有可视化）
