...
# 加载手电筒的资产
flashlight_asset_root = "/home/hun/IsaacGym_Preview_4_Package/isaacgym/flashlight/urdf"
flashlight_asset_file = "flashlight.urdf"
flashlight_asset_options = gymapi.AssetOptions()
flashlight_asset_options.fix_base_link = True  # 根据需要设置是否固定基座
flashlight_asset_options.armature = 0.01
flashlight_asset = gym.load_asset(sim, flashlight_asset_root, flashlight_asset_file, flashlight_asset_options)
...
hand_pose.p = gymapi.Vec3(0.95, 1.25, 0.25)  # 设置手的位置
...
quart_y_flashlight=gymapi.Quat.from_axis_angle(gymapi.Vec3(0,1,0),math.pi/2) #设置手电筒的姿态
...
gym.set_actor_dof_states(env, hand_handle, dof_states, gymapi.STATE_ALL)
# 设置手电筒的位置和姿态
flashlight_pose = gymapi.Transform()
flashlight_pose.p = gymapi.Vec3(1, 1, 0.25)  # 设置手电筒的位置
#flashlight_pose.r = quart_y_flashlight #设置手电筒的姿态
flashlight_handle = gym.create_actor(env, flashlight_asset, flashlight_pose, "flashlight_actor", 50, 1)
flashlight_handles.append(flashlight_handle)

# 获取手和手电筒的刚体形状属性
hand_shape_props = gym.get_actor_rigid_shape_properties(env, hand_handle)
#flashlight_shape_props = gym.get_actor_rigid_shape_properties(env, flashlight_handle)
...
# # 设置手电筒刚体属性
# for prop in flashlight_shape_props:
#     prop.filter = 0b0010  # 电钻碰撞组为2，二进制表示
#     prop.restitution = 0.1  # 设置恢复系数
#     prop.friction = 0.5  # 设置摩擦系数
#     prop.contact_offset = 0.00  # 设置接触偏移量
#     prop.rest_offset = 0.00  # 设置静止偏移量

# 应用修改后的属性
gym.set_actor_rigid_shape_properties(env, hand_handle, hand_shape_props)
#gym.set_actor_rigid_shape_properties(env, flashlight_handle, flashlight_shape_props)
...
print_rigid_body_names(env, flashlight_handle)
...
flashlight_rb_states = gym.get_actor_rigid_body_states(env, flashlight_handle, gymapi.STATE_ALL)

# 打印第一个刚体的位置信息
print("Hand Rigid Body Position:", hand_rb_states['pose']['p'][0])
print("Flashlight Rigid Body Position:", flashlight_rb_states['pose']['p'][0])
#手腕关键点
...
#功能手指关键点
hand_position_F=list(hand_rb_states['pose']['p'][12])
...
#物体上功能手指关键点
flashlight_position_F_O=list(flashlight_rb_states['pose']['p'][3])
flashlight_position_F_O[0]-=0.0

flashlight_position_F_O=tuple(flashlight_position_F_O)

#物体上小拇指关键点
flashlight_position_L_O=list(flashlight_rb_states['pose']['p'][2])
flashlight_position_L_O[0]+=0.01
flashlight_position_L_O[1]+=0.01
flashlight_position_L_O[2]+=0.01

flashlight_position_L_O=tuple(flashlight_position_L_O)

#物体上手腕关键点对应的真实关键点
flashlight_position_W_O=list(flashlight_rb_states['pose']['p'][1])
flashlight_position_W_O[0]+=0.095
flashlight_position_W_O[1]+=0.01
flashlight_position_W_O[2]-=0.04

flashlight_position_W_O=tuple(flashlight_position_W_O)

#物体手腕关键点
flashlight_position_W_O_T=list(flashlight_rb_states['pose']['p'][1])
flashlight_position_W_O_T[0]+=0.02
flashlight_position_W_O_T[1]+=0.01
flashlight_position_W_O_T[2]+=0.01
flashlight_position_W_O_T=tuple(flashlight_position_W_O_T)
...
#在工具上添加标记
add_marker(env, flashlight_position_F_O)
add_marker(env,flashlight_position_L_O)
add_marker(env,flashlight_position_W_O)
add_marker(env,flashlight_position_W_O_T)

#计算手与工具之间的变换矩阵
#先计算旋转矩阵
#建立物体坐标系，
# 两个基轴
origin_o_x=np.array(flashlight_position_F_O)-np.array(flashlight_position_W_O)
origin_o_y=np.array(flashlight_position_L_O)-np.array(flashlight_position_W_O)
#标准化
...
#求解O系和H系间的平移向量
#平移向量在I下的表示
pingyi_O_H_I=np.array(hand_position_W)-np.array(flashlight_position_W_O)
#平移向量在O系下的表示
...
def move_hand_to_target(hand_pose, rotation_O_H, pingyi_O_H_O):
    ...
    hand_pose.p = gymapi.Vec3(
        hand_pose.p.x - pingyi_O_H_I[0]+0.02,
        hand_pose.p.y - pingyi_O_H_I[1]+0.06,
        hand_pose.p.z - pingyi_O_H_I[2]+0.01
    )
...
# 设置手部所有 DOF 的目标位置
#gym.set_actor_dof_position_targets(env, hand_handle, dof_position_targets)
...
i=0
# 主循环
lower_limits[current_dof]+=0.5
lower_limits[zhijian]+=0.1
lower_limits[shizhi_guanjie1]-=0.03
lower_limits[zhongzhi_guanjie1]-=0.3
lower_limits[wumingzhi_guanjie1]-=0.3
lower_limits[xiaomuzhi_guanjie1]-=0.3
...
intermediate_point_x=1.15999995
intermediate_point_y=1.3520005
intermediate_point_z=0.37171
intermediate_point=(intermediate_point_x,intermediate_point_y,intermediate_point_z)
intermediate_point1_x=1.18099995
intermediate_point1_y=1.29620005
intermediate_point1_z=0.37171
...
while not gym.query_viewer_has_closed(viewer):
    ...
    draw_coordinate_frame(gym, viewer, env, hand_position, hand_rotation_matrix, color=(1, 0, 0, 0, 1, 0, 0, 0, 1))  # 绿色

    # 手电筒坐标系
    flashlight_position = np.array(flashlight_position_W_O)
    flashlight_rotation_matrix = rotation_I_O
    draw_coordinate_frame(gym, viewer, env, flashlight_position, flashlight_rotation_matrix, color=(1, 0, 0,0,1,0,0,0,1))  # 红色
    ...
    wucha = (1e-4)*3
    if np.allclose(current_position1[0],target[0],atol=wucha):
